...
const getMonthlyReportsCount = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching reports count" });
  }
};
...
const getEmployee = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching employee count" });
  }
};
...
const getEmployeeSalary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "An error occurred while fetching total employee salary",
      });
  }
};
...
const getMonthlySummary = async (req, res) => {
  try {
    ...
    const totalSalariesResult = await employeesCollection
      .aggregate([
        {
    ...
    const reportsCollection = db.collection("reports");
    const totalCostResult = await reportsCollection
      .aggregate([
        {
          $match: {
            garageId: new ObjectId(garageId),
    ...
    console.log(
      "Adjusted Total Cost after subtracting Garage Cost:",
      adjustedCost
    );
...
const getModelsSummary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching models summary" });
  }
};
...
module.exports = {
  getMonthlyReportsCount,
  getEmployee,
  getEmployeeSalary,
  getMonthlySummary,
  getModelsSummary,
  getTopEmployeesThisMonth,
  getReports,
};