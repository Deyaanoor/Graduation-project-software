...
const getContactMessagesByUserId = async (req, res) => {
  try {
    ...
    const messages = await contactMessagesCollection.find({ user_id: new ObjectId(userId) }).toArray(); // جلب الرسائل بناءً على userId
...
const getContactMessages = async (req, res) => {
    try {
      ...
      const messages = await contactMessagesCollection.find().toArray(); // جلب جميع الرسائل
      res.status(200).json(messages); // إرجاع جميع الرسائل
    } catch (error) {
      console.error('❌ Error fetching messages:', error);
      res.status(500).json({ message: 'An error occurred while fetching messages' });
    }
...
const updateContactMessageStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'An error occurred while updating the message status' });
  }
};
...
const deleteContactMessage = async (req, res) => {
  try {
    ...
    const user = await userCollection.findOne({ _id: new ObjectId(message.user_id) });
